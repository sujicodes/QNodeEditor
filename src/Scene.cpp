...
void Scene::initUI()
{
 ...
 NodeItem* Scene::createNode(QString type)
{
    NodeItem* node = NodeRegistry::instance().createNode(
                type,
                this
            );
...
void Scene::addNode(NodeItem* node)
{
    nodes.append(node);
}
...
void Scene::removeNode(NodeItem* node)
{
...
bool Scene::loadFromFile(const QString& filename,  QString* errorMsg)
{
    ...
    if(!doc.isObject())
    {
        if(errorMsg) *errorMsg = QString("Invalid JSON format: root is not an object");
        return false;
    }
...
NodeItem* Scene::getNodeById(qint64 id) const
{
...
void Scene::resetLastSelectedStates()
{
    for(auto* node : nodes)
    {
        if(node && node) node->setLastSelectedState(false);
    }

    for(auto* edge : edges)
    {
        if(edge && edge->getEdgeGraphicsItem()) edge->getEdgeGraphicsItem()->setLastSelectedState(false);
    }
}
...
{
    if(_graphicsScene->views().isEmpty()) return nullptr;
